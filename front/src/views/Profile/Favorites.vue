<template>
  <div class="favorites-container">
    <div class="article-list" v-loading="loading">
      ...
      <!-- 空状态 -->
      <el-empty 
        v-if="favoriteArticles.length === 0 && !loading" 
        description="暂无收藏文章"
      >
    ...
    <!-- 分页器 -->
    <div class="pagination" v-if="favoriteArticles.length > 0">
      <el-pagination
...
const loadFavoriteArticles = async () => {
  ...
  console.log('开始加载收藏文章列表，用户ID:', userId.value)
  loading.value = true
  
  try {
    const response = await request.get<Article[]>('/server/personalCenterData/getCollectArticles', {
      params: { userId: userId.value }
    })
    
    console.log('获取收藏文章列表成功:', response.data)
    
    if (Array.isArray(response.data)) {
      favoriteArticles.value = response.data
      total.value = response.data.length
      console.log(`共加载 ${total.value} 篇收藏文章`)
